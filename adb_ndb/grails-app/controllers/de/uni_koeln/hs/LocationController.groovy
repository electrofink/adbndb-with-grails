...
class LocationController {
	...
	def save = {
		...
		if (locationInstance.save(flush: true)) {
			...
			personLocationsInstance.person = flash.person as Person
			personLocationsInstance.location = locationInstance
			...
			if (params.version) {
				...
				if (locationInstance.version > version) {
					...
					locationInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [
						message(code: 'location.label', default: 'Location')]
					as Object[], "Another user has updated this Location while you were editing")
